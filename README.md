...
`make run` (которая запускает команду `docker-compose up -d`). 

Если вы хотите чтобы приложение не работало в фоне и было видно, что происходит, выполните 
команду `docker-compose up`.

В результате которой поднимаются БД Postgres, Rabbit MQ и накатываются миграции на БД
Сервер GRPC готов принимать запросы на порту 8086.
Наружу у БД открыт порт 5472, у Rabbit MQ 15672 (здесь работает админка).

Для того чтобы снаружи подключиться к указанным выше портам необходимо для определения ip адреса выполнить команду
`docker-machine ip default`


Все настройки для подключения к БД лежат в файле envs/database.env
Все настройки для подключения к Rabbit MQ лежат в файле envs/queue.env

Описание grpc сервиса находится в файле api/RotatorService.proto
## Остановка приложения
Происходит с помощью команды make stop (которая запускает команду docker-compose down)
## Проверка приложения линтером
`make lint`
## Запуск unit тестов
`make test`
...
<img src="./resources/conceptual_model.png" width="600">
